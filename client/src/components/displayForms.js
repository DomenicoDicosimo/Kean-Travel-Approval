...
import {
  ...
  formatTime,
  formatDate,
} from './DisplayStudentTravelRegistrationFormDay.js';
...
const DisplayForms = ({ formData }) => {
  ...
  function downloadPDF() {
    html2canvas(formRef.current, { scale: 3 }).then((canvas) => {
      ...
      pdf.save(`${formData.first_name}${formData.last_name}-${formData.event_name}.pdf`);
    });
  }
            ...
            {formData.isUnderage && (
              <Box>
                ...
                <FormControl>
                  ...
                  <Input
                    value={formatPhoneNumber(formData.parent_contact_number) || ''}
                    isReadOnly
                  />
                </FormControl>
              </Box>
